            ...
            <form method="post" id="productSearchForm">
                ...
                <div class="row">
                    <div class="col-md-3 mb-4">
                        ...
                        <input type="text" name="product_name" placeholder="Type prodcut name here" class="form-control"
                            id="order_id">
                    </div>
                    <div class="col-md-3 mb-4">
                        <label for="order_customer" class="form-label">Product Sku Code</label>
                        <input type="text" name="sku" placeholder="Type product sku code here" class="form-control"
                            id="order_customer">
                    </div>
                    <div class="col-md-3 mb-4">
                        <label for="order_created_date" class="form-label">Starting Date</label>
                        <input type="date" placeholder="Type created date here" name="created_at" class="form-control"
                            id="order_created_date">
                    </div>
                    <div class="col-md-3 mb-4">
                        <label for="order_created_date" class="form-label">Ending Date</label>
                        <input type="date" placeholder="Type updated  date here" name="updated_at" class="form-control"
                            id="order_created_date">
                    </div>
                    <div class="col-md-3 mb-4">
                            ...
                            <td class="text-end">
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                        ...
                                        <a class="dropdown-item" href="{{ route('products.edit', $product->id) }}">Edit
                                            info</a>
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        products.forEach(function(product, index) {
                            ...
                            if (product.overviews && product.overviews.length > 0) {
                                var overviews = product.overviews.map(function(
                                overview) {
                                    return overview.overview_name + ' ' +
                                        overview.overview_value;
                                }).join('<br>');
                                row.append($('<td>').html(overviews));
                            } else {
                                row.append($('<td>').text('No overviews available'));
                            }

                            // Check the product status and append the corresponding badge
                            ...
                            var detailLink = $('<a>').attr('href',
                                "{{ route('products.show', ':slug') }}".replace(
                                    ':slug', product.slug)).addClass(
                                'btn btn-md rounded font-sm').text('Detail');
                            var dropdown = $('<div>').addClass('dropdown');
                            var dropdownButton = $('<a>').attr('href', '#').attr(
                                'data-bs-toggle', 'dropdown').addClass(
                                'btn btn-light rounded btn-sm font-sm').html(
                                '<i class="material-icons md-more_horiz"></i>');
                            var dropdownMenu = $('<div>').addClass('dropdown-menu');
                            var editLink = $('<a>').attr('href',
                                    "{{ route('products.edit', ':id') }}".replace(
                                        ':id', product.id)).addClass('dropdown-item')
                                .text('Edit info');
                            var deleteLink = $('<a>').attr('href',
                                "{{ route('products.destroy', ':id') }}".replace(
                                    ':id', product.id)).addClass(
                                'dropdown-item text-danger').text('Delete');
